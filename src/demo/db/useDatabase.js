 ...
 * - Exposes runDiscovery(), which finds every dataset in the table and returns one tree's rows per dataset — no scenario names hardcoded
 *
 *   runDiscovery, returns [{ name, rows }] per dataset found
 */
  ...
  const runDiscovery = useCallback(async () => {
    ...
    try {
      ...
      for (const required of ['id', 'name', 'parentId']) 
      {
        ...
        {
          setError(`The uploaded file has no "${required}" column. ` + `Columns found: ${columns.join(', ')}. ` + `Files need id, name and parentId to form a tree.`);
